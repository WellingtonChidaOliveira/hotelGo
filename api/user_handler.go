...
func (h *UserHandler) HandlerPutUser(c *fiber.Ctx) error {
	...
	return c.JSON(map[string]string{"message": fmt.Sprintf("user with id %s updated", id)})
}
...
func (h *UserHandler) HandlerDeleteUser(c *fiber.Ctx) error {
	...
	return c.JSON(map[string]string{"message": fmt.Sprintf("user with id %s deleted", id)})
}
